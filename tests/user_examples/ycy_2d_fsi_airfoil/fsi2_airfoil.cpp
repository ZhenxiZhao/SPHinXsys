/**
 * @file 	fsi2_airfoil.cpp
 * @brief 	This is the case for revise my cpc paper (level-set cleaning).
 * @details considering a very sharp airfoil shap interact with fluid, 
 *			to check if the un-resolved small structure infect the simulation or not.
 * @author 	Yongchuan Yu
 */
#include "sphinxsys.h"

#include "fsi2_airfoil.h" //	case file to setup the test case
using namespace SPH;
...
// handle command line arguments
	...
	/*relaxation with clean or not, you need to shift between line 44 and line 45*/
	insert_body.defineBodyLevelSetShape()->cleanLevelSet()->writeLevelSet(io_environment);
	...
	/*if you want to try the lattice particle distribution, you need comment the line 47-49, and run line 52*/
	//insert_body.generateParticles<ParticleGeneratorLattice>();

	ObserverBody airfoil_observer(sph_system, "AirfoilObserver");
	airfoil_observer.generateParticles<ObserverParticleGenerator>(airfoil_observation_location);
	...
	ContactRelation airfoil_observer_contact(airfoil_observer, { &insert_body });
	...
	insert_body.addBodyStateForRecording<Vecd>("Acceleration");
	//----------------------------------------------------------------------
	...
	BodyRegionByParticle airfoil_base(insert_body, makeShared<MultiPolygonShape>(createAirfoilBaseShape()));
	SimpleDynamics<solid_dynamics::FixBodyPartConstraint> constrain_airfoil_base(airfoil_base);
	/** Update norm .*/
	...
	ObservedQuantityRecording<Vecd> write_airfoil_tip_displacement("Position", io_environment, airfoil_observer_contact);
	...
	write_airfoil_tip_displacement.writeToFile(number_of_iterations);
	//----------------------------------------------------------------------
