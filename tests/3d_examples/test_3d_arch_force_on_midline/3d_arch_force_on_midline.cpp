...
Real physical_viscosity = 00.0; /** physical damping, here we choose the same value as numerical viscosity. */

Real time_to_full_external_force = 0.001;
Real gravitational_acceleration = -300000.*4;
Real F_full = -10;

/*point force*/
std::vector<Vec3d> point_force_shell{ Vec3d(0.0, 0.0,F_full) };
std::vector<Vec3d> reference_position_shell_point{ Vec3d(0.0, 0.0, 0.0) };
	...
	virtual void initializeGeometricVariables() override
	{
		...
		for (int i = 0; i < particle_number_mid_surface + 2 * BWD; i++)
		{
			for(int j = 0; j < particle_number; j++)
			{
				...
				if(i == int((particle_number_mid_surface + 2 * BWD) / 2.)){

					reference_position_shell_point.push_back(Vecd(x, y, z));
					point_force_shell.push_back(Vec3d(0, 0, F_full));
...
int main()
{
	...
	SimpleDynamics<thin_structure_dynamics::DistributingPointForcesToShell> apply_point_force_shell(cylinder_body, point_force_shell, reference_position_shell_point, time_to_full_external_force, particle_spacing_ref);
	/**
	...
	while (GlobalStaticVariables::physical_time_ < end_time)
	{
		...
		while (integral_time < output_period)
		{
			...
			apply_point_force_shell.parallel_exec(dt);
