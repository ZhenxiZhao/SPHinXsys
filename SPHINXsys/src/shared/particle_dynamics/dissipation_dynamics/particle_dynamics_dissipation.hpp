...
namespace SPH
{
	...
	template <typename DataType, class CoefficientType>
	template <typename CoefficientArg>
	BaseDampingSplittingWithWallFlux<DataType, CoefficientType>::
		BaseDampingSplittingWithWallFlux(ComplexRelation &complex_wall_relation,
			                             const std::string &variable_name, const std::string &flux_name,
			                             const std::string &normal_vector_name, const CoefficientArg &eta)
		: BaseDampingSplittingWithWall<DataType, CoefficientType>(complex_wall_relation, variable_name, eta)
	{
		for (size_t k = 0; k != this->DissipationDataWithWall::contact_particles_.size(); ++k)
		{
			wall_flux_.push_back(this->contact_particles_[k]->template getVariableByName<Real>(flux_name));
			wall_normal_vector_.push_back(this->contact_particles_[k]->template getVariableByName<Vecd>(normal_vector_name));
		}
	}
	//=================================================================================================//
	template <typename DataType, class CoefficientType>
	...
	{
		ErrorAndParameters<DataType> error_and_parameters =
			BaseDampingSplittingWithWall<DataType, CoefficientType>::computeErrorAndParameters(index_i, dt);
		
		Real Vol_i = this->Vol_[index_i];
		/** Contact interaction. */
		for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
		{
			StdLargeVec<Real> &flux_k = *(this->wall_flux_[k]);
			StdLargeVec<Vecd>& normal_vector_k = *(this->wall_normal_vector_[k]);
			Neighborhood& contact_neighborhood = (*DissipationDataWithWall::contact_configuration_[k])[index_i];
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
			{
				...
				//linear projection
				Real variable_derivative = -flux_k[index_j] * contact_neighborhood.r_ij_[n] /
					this->coefficient_(index_i) / normal_vector_k[index_j].dot(contact_neighborhood.e_ij_[n]);
